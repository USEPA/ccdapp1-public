...
public class SearchChemicalService {
    ...
    public String preprocessingSearchWord(String searchWord){
        ...
        if(isCasrn(searchWord)){
            ...
            // interchangeable apostrophes CE-3198
            searchWord = searchWord.replaceAll("`","'"); // ASCII - 96
            searchWord = searchWord.replaceAll("’","'"); // ASCII - 146
...
// Check digit is the last char, compare to sum mod 10.
    ...
    public List<ChemicalLongResult> removeDuplicatesForLong(List<ChemicalLongResult> chemicals) {

        List<ChemicalLongResult> returnList = new ArrayList<ChemicalLongResult>();
        List<String> dtxsidList = new ArrayList<String>();
        List<String> dtxcidList = new ArrayList<String>();

        for(ChemicalLongResult chemical : chemicals){
    ...
    public List<ChemicalShortResult> removeDuplicatesForShort(List<ChemicalShortResult> chemicals) {

        List<ChemicalShortResult> returnList = new ArrayList<ChemicalShortResult>();
        List<String> dtxsidList = new ArrayList<String>();
        List<String> dtxcidList = new ArrayList<String>();

        for(ChemicalShortResult chemical : chemicals){
            if(chemical.getDtxsid() != null ){}
            if(dtxsidList.contains(chemical.getDtxsid()) == false){
                dtxsidList.add(chemical.getDtxsid());
                returnList.add(chemical);
    ...
    public List<SearchChemicalDetails> removeDuplicatesForChemicalDetails(List<SearchChemicalDetails> chemicals) {
        List<SearchChemicalDetails> returnList = new ArrayList<SearchChemicalDetails>();
        List<String> dtxsidList = new ArrayList<String>();
        List<String> dtxcidList = new ArrayList<String>();

        for(SearchChemicalDetails chemical : chemicals){
            if(chemical.getDtxsid() != null ){}
            if(dtxsidList.contains(chemical.getDtxsid()) == false){
                dtxsidList.add(chemical.getDtxsid());
                returnList.add(chemical);
